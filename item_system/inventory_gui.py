...
class Inventory_GUI:
    def __init__(self, list_items: [], list_items_to_trade: [] = None):
        self.list_items_to_trade = list_items_to_trade
    ...
    def draw(self, chosen_item: int = None) -> BytesIO:
        ...
        # Trade Panel drawing
        if self.list_items_to_trade is not None:
            ...
            for i in range(self.panel_items_size):
                if i < len(self.list_items_to_trade):
                    item_holder = Image.open(str(self.dict_rarity[self.list_items_to_trade[i].rarity]))

                    item_holder = item_holder.resize(
                        (int(height * self.panel_item_holders_size_k), int(height * self.panel_item_holders_size_k)))


                    item_img = Image.open(self.list_items_to_trade[i].img_url)
